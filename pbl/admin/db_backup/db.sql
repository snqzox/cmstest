CREATE DATABASE IF NOT EXISTS `pbl123` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
    ...
    CREATE TABLE IF NOT EXISTS `service` (
      ...
      FOREIGN KEY (`page_id`) REFERENCES pageServices(`ID`)
        ...
        CREATE TABLE IF NOT EXISTS `article` (
          ...
          PRIMARY KEY (`ID`),
          FOREIGN KEY (`subservice_id`) REFERENCES service(`ID`)
        ) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=5;
  ...
  `subtitle` varchar(500) NOT NULL,
   PRIMARY KEY (`ID`)
      ...
      CREATE TABLE IF NOT EXISTS `studio` (
        ...
        CREATE TABLE IF NOT EXISTS `defaultRef` (
            `ID` mediumint(9) NOT NULL AUTO_INCREMENT,
  ...
  `dic` varchar(100) NOT NULL,
   PRIMARY KEY (`ID`)
        ...
        `defaultRef_id` mediumint(9)DEFAULT NULL,
         ...
         FOREIGN KEY (`subsidy_id`) REFERENCES subsidy(`ID`) on delete cascade,
         FOREIGN KEY (`studio_id`) REFERENCES studio(`ID`) on delete cascade,               
         FOREIGN KEY (`activity_id`) REFERENCES activity(`ID`) on delete cascade,               
         FOREIGN KEY (`defaultRef_id`) REFERENCES defaultRef(`ID`) on delete cascade
      ) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=5;
...
INSERT INTO service (id, title, subtitle, content, subservice, page_id)
...
INSERT INTO article (id, title, subtitle, content, subservice_id)
